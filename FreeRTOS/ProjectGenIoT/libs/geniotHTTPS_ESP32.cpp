 ...
 * Construtor
 */
Geniot::Geniot(char* token_geniot) {
...
bool Geniot::read( char * variable_id ) {
    bool retorno = false; 
    if( (wifiMulti.run() == WL_CONNECTED) ) {
                    ...
                    cJSON_ArrayForEach(resolution, resolutions)
                    {
                       ...
                       cJSON *description = cJSON_GetObjectItem((cJSON*)resolution, "description");
                        ...
                        if((description->valuestring != NULL)){
                          ...
                          sprintf( geniotText, "%s",description->valuestring );
                        }

                          if( DEBUG )
                            vPrintStringAndFloat( (char*) "Value: ", value->valuedouble );

                          geniotValue = value->valuedouble; 
                          retorno = true;
...
void Geniot::vPrintString( const char *pcString ){
...
void Geniot::vPrintStringAndFloat( const char *pcString, float ulValue ){
  taskENTER_CRITICAL( &myMutex );
  {
    char buffer [50]; 
...
void Geniot::vPrintStringAndInteger( const char *pcString, uint32_t ulValue ){
  taskENTER_CRITICAL( &myMutex );
  {
    char buffer [50]; 
...
void Geniot::vPrintTwoStrings(const char *pcString1, const char *pcString2){
  taskENTER_CRITICAL( &myMutex  );
  {
    char buffer [50]; 
